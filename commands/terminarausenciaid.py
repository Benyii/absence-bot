...
async def terminarausenciaid(interaction: discord.Interaction, absence_id: int):
    try:
        async with aiosqlite.connect('absences.db') as db:
            async with db.execute('SELECT user_id, start_date, end_date, status, reason, public_message_id, approved_message_id FROM absences WHERE id = ? AND status = ?', (absence_id, 'approved')) as cursor:
            ...
            user_id, start_date, end_date, status, reason, public_message_id, approved_message_id = row
